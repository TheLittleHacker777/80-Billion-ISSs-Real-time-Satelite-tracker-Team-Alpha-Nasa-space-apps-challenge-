window.ISS = class {
  ...
  static createSpacecraft(satelliteTracker, tle, progressCallback) {
    ...
    satellite.dock(
      ...
      null,
      "solar_array_rod",
      function (model, date, satellite) {
        model.shift(
          new THREE.Vector3(0.0, 5.0, 5.0),
          ISS.calculateSolarArrayRotation(date, satellite, false)
        );
      }
    );
    ...
    for (let i = 0; i < 2; i++) {
      ...
      satellite.dock(
        ...
        "Solar Array",
        null,
        i === 0 ? "solar_array_left" : "solar_array_right",
        function (model, date, satellite) {
          model.shift(
            new THREE.Vector3(50.0 * mirror, 5, 5.0),
            ISS.calculateSolarArrayRotation(date, satellite, true)
          );
        }
      );
      ...
      satellite.dock(
        ...
        function (model, date, satellite) {
          model.shift(
            new THREE.Vector3(34.3 * mirror, 5, 5.0),
            ISS.calculateSolarArrayRotation(date, satellite, true)
          );
        }
      );
  ...
  static calculateSolarArrayRotation(date, satelliteObject, twoAxes = true) {
    ...
    // Sun position
    let sunCoordinates = latLonDegToVector3(
      toDegrees(sunPosition.lat),
      toDegrees(sunPosition.lng) + 90 - posAndRot.position.longitude + 90,
      100
    );

    // Earth rotation
    sunCoordinates.applyAxisAngle(
      new THREE.Vector3(1, 0, 0),
      toRadians(-posAndRot.position.latitude + 90)
    );
