window.SatelliteModel = class {
  ...
  constructor(
    ...
    // Marker circle
    const markerTextureMap = satelliteTracker.textureLoader.load(
  ...
  update(date, posAndRot, showLabels, satellite, focused, showModel) {
    ...
    this.label.position.set(
      posAndRot.position.x,
      posAndRot.position.y,
      posAndRot.position.z
    );
    this.label.rotation.set(
      posAndRot.rotation.x,
      posAndRot.rotation.y,
      posAndRot.rotation.z
    );
    ...
    if (showLabels) {
      ...
      for (let i = 0; i <= ISS_ORBIT_TIME; i += 1) {
        ...
        let position = latLonDegToVector3(
          state.latitude,
          state.longitude + 90,
          state.getDistanceToEarthCenter()
        );
  ...
  loadModel(id, state, successCallback, progressCallback, errorCallback) {
    this.satelliteTracker.gltfLoader.load(
      "assets/models/" + id + "/" + state + ".glb",
      successCallback,
      function (xhr) {
