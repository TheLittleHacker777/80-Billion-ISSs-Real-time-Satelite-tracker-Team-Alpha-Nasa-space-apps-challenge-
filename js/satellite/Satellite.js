window.Satellite = class {
  ...
  getPositionAtTime(date = new Date()) {
    ...
    if (positionEci === undefined) {
      return new SatellitePositionAtTime(
        Number.NaN,
        Number.NaN,
        Number.NaN,
        new THREE.Vector3()
  ...
  getPositionAndRotationAtTime(date = new Date()) {
    ...
    // Get rotation
    let rotation = lookAtThreeJs(
      positionData.coordinatePosition,
      positionData.prevCoordinatePosition
    );
  ...
  getPositionData(date = new Date()) {
    ...
    let coordinatePosition = latLonDegToVector3(
      position.latitude,
      position.longitude + 90,
      position.getDistanceToEarthCenter()
    );
    let prevCoordinatePosition = latLonDegToVector3(
      prevPosition.latitude,
      prevPosition.longitude + 90,
      prevPosition.getDistanceToEarthCenter()
    );
  ...
  dock(
    ...
    this.docking.push(
      new SatelliteModel(
        ...
        state,
        name,
        SatelliteModel.EMPTY_LOAD_CALLBACK,
        port,
        animationCallback
      )
    );
