window.Renderer = class {
  ...
  init(spaceScene, hudScene) {
    ...
    this.webRenderer = this.supportWebGL
    ...
    document
      .getElementById(this.canvasWrapperId)
  ...
  registerListener() {
    ...
    window.addEventListener(
      "resize",
      (event) => scope.onWindowResize(event),
      false
    );
    ...
    dom.addEventListener(
      "mousedown",
      (event) => scope.onMouseClick(event),
      true
    );
    dom.addEventListener(
      "mouseup",
      (event) => scope.onMouseRelease(event),
      true
    );
    dom.addEventListener(
      "touchstart",
      (event) => scope.onTouchStart(event),
      true
    );
    dom.addEventListener("touchend", (event) => scope.onTouchEnd(event), true);
    dom.addEventListener(
      "mousemove",
      (event) => scope.onMouseMove(event),
      true
    );
  ...
  onWindowResize() {
    ...
    this.sceneHUD = this.satelliteTracker.hudScene.createHUDScene(
      this.hudCanvas,
      this.cameraHUD
    );
  ...
  onMouseClick(event) {
    ...
    this.mouse.x =
      (event.clientX / this.webRenderer.domElement.clientWidth) * 2 - 1;
    this.mouse.y =
      -(event.clientY / this.webRenderer.domElement.clientHeight) * 2 + 1;
  ...
  onKeyDown(event) {
    this.satelliteTracker.hudScene.onKeyDownScreen(
      event.key,
      event.keyCode,
      event.ctrlKey
    );
