window.SpaceScene = class {
  ...
  createSpaceScene(camera) {
    ...
    // Ambient Light
    const nightLight = new THREE.AmbientLight(
      0x888888,
      this.satelliteTracker.debug ? 3.2 : 0.1
    );
    ...
    // Add sun flare
    let textureFlareLens =
      this.satelliteTracker.textureRegistry.get("lens_flare");
    let textureFlareSun =
      this.satelliteTracker.textureRegistry.get("sun_flare");
    ...
    lensflare.addElement(
      new THREE.LensflareElement(textureFlareLens, 120, 0.9)
    );
    ...
    earthMaterial.bumpMap =
      this.satelliteTracker.textureRegistry.get("earth_bump");
    earthMaterial.normalMap =
      this.satelliteTracker.textureRegistry.get("earth_normal");
    earthMaterial.specularMap =
      this.satelliteTracker.textureRegistry.get("earth_spec");
    ...
    this.createNightLightMaterial(function (nightLightMaterial) {
      scope.nightSide = new THREE.Mesh(
        new THREE.SphereBufferGeometry(EARTH_RADIUS + 1000, 64, 64),
        nightLightMaterial
      );
    ...
    // Clouds
    const cloudsGeometry = new THREE.SphereBufferGeometry(
      EARTH_RADIUS + CLOUD_HEIGHT,
      64,
      64
    );
    ...
    const atmosphereGeometry = new THREE.SphereGeometry(
      EARTH_RADIUS + ATMOSPHERE_HEIGHT,
      256,
      256
    );
    ...
    // Stars
    const starsGeometry = new THREE.SphereBufferGeometry(
      SUN_DISTANCE * 2,
      32,
      32
    );
    ...
    starsMaterial.map =
      this.satelliteTracker.textureRegistry.get("galaxy_starfield");
  ...
  updateSpace(date, layers) {
    ...
    Object.values(this.satelliteTracker.registry.satellites).forEach(
      (satellite) => {
        satellite.updateModel(
          date,
          !canSeeFocusedSatellite,
          satellite.id === focusedSatellite.id
        );
      }
    );
    ...
    let sunCoordinates = latLonRadToVector3(
      sunPosition.lat,
      sunPosition.lng + toRadians(90),
      SUN_DISTANCE
    );
    ...
    let moonPosition = latLonDegToVector3(
      moonState.latitude,
      moonState.longitude + 90,
      moonState.altitude
    );
    ...
    if (this.nightSide != null) {
      this.nightSide.material.uniforms.viewVector.value =
        sunCoordinates.normalize();
    }
  }
  ...
  focusSatellite(
    ...
    this.centerGroup.position.set(
      0,
      -posAndRot.position.getDistanceToEarthCenter(),
      0
    );

    // Rotate the earth with the ISS position to the top
    if (this.satelliteTracker.focusedEarth) {
      this.earthGroup.rotation.y = toRadians(
        ((date.getTime() % DAY_IN_MS) / DAY_IN_MS) * 360
      );
    } else {
      // Rotate earth
      this.earthGroup.rotation.x = toRadians(-posAndRot.position.latitude + 90);
      this.earthGroup.rotation.y = toRadians(
        -posAndRot.position.longitude + 90
      );
    }
    ...
    this.clouds.rotation.y = toRadians(
      ((date.getTime() % (DAY_IN_MS * 2)) / (DAY_IN_MS * 2)) * 360
    );

    // Update controls
    this.renderer.controls.zoomSpeed =
      cameraDistance < 200 || cameraDistance >= EARTH_RADIUS ? 1 : 8;
    this.renderer.controls.target = new THREE.Vector3(
      0,
      this.satelliteTracker.focusedEarth ? this.centerGroup.position.y : 0,
      0
    );
    this.renderer.controls.minDistance = this.satelliteTracker.focusedEarth
      ? EARTH_RADIUS + ATMOSPHERE_HEIGHT * 8
      : 10;

    // Update bump scale of earth
    this.earth.material.bumpScale =
      cameraDistance < EARTH_RADIUS ? 1000 : 10000;
  ...
  updateAtmosphere(cameraDistance, posAndRot) {
    ...
    let cameraVector = new THREE.Vector3(
      this.renderer.camera.matrix.elements[8],
      this.renderer.camera.matrix.elements[9],
      this.renderer.camera.matrix.elements[10]
    );

    // Distance to the configured shader setting (The atmosphere shader is configured on a altitude of 437 km)
    let distanceToAtmosphere = this.satelliteTracker.focusedEarth
      ? cameraDistance
      : cameraDistance + posAndRot.position.altitude * 1000 - 437000;

    // Calculate fade out for atmosphere strength
    let strength =
      (ATMOSPHERE_STRENGTH -
        (ATMOSPHERE_STRENGTH / this.renderer.controls.maxDistance) *
    ...
    this.atmosphere.material.uniforms.viewVector.value = this.satelliteTracker
      .focusedEarth
      ? cameraVector
      : new THREE.Vector3(0, 1, 0);
    this.atmosphere.material.uniforms.strength.value = Math.min(
      Math.max(strength, 0.0),
      ATMOSPHERE_STRENGTH
    );
    this.atmosphere.material.uniforms.shift.value = Math.max(
      0,
  ...
  createNightLightMaterial(callback) {
    ...
    let nightTexture =
      scope.satelliteTracker.textureRegistry.get("earth_night_map");
