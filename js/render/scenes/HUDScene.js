window.HUDScene = class {
  ...
  updateHUD(date, mouseX, mouseY) {
    // Not initialized yet
    if (this.context == null || this.hudTexture == null) return;
    ...
    if (this.satelliteTracker.loadingProgress.isReady()) {
      ...
      if (focusedSatellite !== undefined) {
        ...
        if (!this.satelliteTracker.renderer.isMobile) {
          ...
          if (
            !this.satelliteTracker.focusedEarth &&
            !this.hoverToggleEarthFocusButton
          ) {
            this.drawHint(
              width - 40,
              height - 40,
              "You can switch to earth view here",
              "#666666",
              "#ffffff",
              0.4
            );
      ...
      // Draw satellite list
      this.drawSatelliteList(
        3,
        height - (this.satelliteTracker.renderer.isMobile ? 10 : 160),
        mouseX,
        mouseY
      );
      ...
      if (this.flagAddSatelliteMenuOpen) {
        this.drawAddSatelliteMenu(
          width / 2,
          height / 6,
          this.satelliteTracker.renderer.isMobile
            ? width - 20
            : Math.max(400, width / 3),
          50,
          mouseX,
          mouseY
        );
  ...
  onMouseMove(clientX, clientY) {
    if (this.timeDragging) {
      ...
      this.satelliteTracker.currentTime = this.satelliteTracker
        .getTime()
        .getTime();
  ...
  onMouseClick(mouseX, mouseY) {
    ...
    if (this.showTimeline && !this.timeDragging && this.hoverLive) {
      ...
      this.satelliteTracker.prevTime = this.satelliteTracker.getBrowserTime();
      this.satelliteTracker.currentTime =
        this.satelliteTracker.getBrowserTime();
    }
    ...
    if (this.hoverAddSatelliteMenu) {
      ...
      if (this.hoverSatelliteTLE != null) {
        this.satelliteTracker.registry.spawnSatellite(
          new Satellite(this.satelliteTracker, this.hoverSatelliteTLE)
        );
    ...
    if (this.hoverSatelliteToRemove != null) {
      this.satelliteTracker.registry.destroySatellite(
        this.hoverSatelliteToRemove
      );
      this.hoverSatelliteToRemove = null;
    }
  }
  ...
  onKeyDownScreen(key, code, ctrlKey) {
    if (this.flagAddSatelliteMenuOpen) {
      if (key.length === 1 && !ctrlKey) {
        ...
        this.stringSearchQuery = this.stringSearchQuery.substr(
          0,
          this.stringSearchQuery.length - 1
        );
      } else if (code === 17) {
        this.stringSearchQuery = "";
      }
  ...
  drawEarthFocusButton(x, y, size, mouseX, mouseY) {
    ...
    if (mouseOver) {
      this.drawRightText(
        x - 5,
        y + size / 2 + 4,
        "Focus " + (this.satelliteTracker.focusedEarth ? "Satellite" : "Earth"),
        "#ffffff",
        20,
        false
      );
  ...
  drawDockingButton(x, y, size, mouseX, mouseY) {
    let mouseOver = (this.hoverToggleDockingButton =
      mouseX > x && mouseX < x + size && mouseY > y && mouseY < y + size);
    let offset = mouseOver ? 1 : 0;
    ...
    if (mouseOver) {
      this.drawRightText(
        x - 5,
        y + size / 2 + 4,
        (this.showDockingInformation ? "Hide" : "Show") +
          " docking information",
        "#ffffff",
        20,
        false
  ...
  drawTimeButton(x, y, size, mouseX, mouseY) {
    let mouseOver = (this.hoverToggleTimeButton =
      mouseX > x && mouseX < x + size && mouseY > y && mouseY < y + size);
    let offset = mouseOver ? 1 : 0;
    ...
    if (mouseOver) {
      this.drawRightText(
        x - 5,
        y + size / 2 + 4,
        (this.showTimeline ? "Hide" : "Show") + " speed settings",
        "#ffffff",
        20,
        false
      );
  ...
  drawSatelliteList(x, y, mouseX, mouseY) {
    ...
    Object.values(this.satelliteTracker.registry.satellites).forEach(
      (satellite) => {
        y -= 32;
  ...
  drawSatelliteEntry(satellite, x, y, height, mouseX, mouseY) {
    ...
    let gradient = this.getGradientTopBottom(
      x,
      y,
      y + height,
      "rgba(0,0,0, " + (mouseOver ? 0.8 : 0.3) + ")",
      "rgba(0,0,0, " + (mouseOver ? 0.6 : 0.2) + ")"
    );
    ...
    this.drawImage(
      iss ? this.textureISS : this.textureSatellite,
      x + gap,
      y + gap,
      height - gap * 2,
      height - gap * 2,
    ...
    this.drawText(
      ...
      satellite.name,
      "rgba(255,255,255, " + (focused ? 1.0 : 0.4) + ")",
      fontSize,
      false
    );
    ...
    if (parseInt(satellite.id) !== ISS.ID) {
      ...
      this.drawRect(
        ...
        x + width + height + 2,
        y + height,
        "rgba(0,0,0, " + (hoverMinus ? 0.6 : 0.2) + ")"
      );
      this.drawImage(
        this.textureMinus,
        x + width + minusOffset + 2,
        y + minusOffset,
        height - minusOffset * 2,
        height - minusOffset * 2,
        hoverMinus ? 1.0 : 0.4
      );
  ...
  drawDockingList(docking, x, y) {
    ...
    for (const index in docking) {
      ...
      if (satellite.port != null) {
        maxValueLength = Math.max(
          maxValueLength,
          this.getStringWidth(satellite.name, fontSize, false)
        );
        maxKeyLength = Math.max(
          maxKeyLength,
          this.getStringWidth(satellite.port.name, fontSize, false)
        );
        list.push(satellite);
      }
    }
    ...
    // Title
    this.drawText(
      x - maxValueLength - 10 - maxKeyLength,
      y,
      "Docked spacecrafts:",
      "#ffffff",
      false
    ...
    list.sort((a, b) => {
      return (
        this.getStringWidth(b.port.name, fontSize, false) -
        this.getStringWidth(a.port.name, fontSize, false)
      );
    });
      ...
      for (let id in this.satelliteTracker.registry.database) {
        ...
        if (
          ...
          if (matches <= 8) {
            ...
            this.drawText(
              x - width / 2 + entryHeight + 4,
              listY + entryHeight - 9,
              name,
              "rgba(255,255,255, " + (hoverEntry ? 0.8 : 0.5) + ")",
              entryHeight / 2,
              false
            );
            this.drawRightText(
            ...
            this.drawImage(
              this.texturePlus,
              x + width / 2 - entryHeight + iconOffset,
              listY + iconOffset,
              entryHeight - iconOffset * 2,
              entryHeight - iconOffset * 2,
              hoverAddButton ? 0.8 : 0.4
            );
  ...
  drawTimeline(x, y, width, height, mouseX, mouseY, date) {
    ...
    let snapAnimationPercentage = Math.min(
      1,
      (1.0 / 100) *
        (this.satelliteTracker.getBrowserTime() -
          this.satelliteTracker.prevTime)
    );
    let snapAnimationSpeedOffset = this.timeDragging
      ? 0
      : (prevSpeed - speed) * (1 - snapAnimationPercentage);
    ...
    for (let index in this.timeline) {
      ...
      let rotationOffset =
        (marker.speed - speed - snapAnimationSpeedOffset) / 10;
      ...
      this.drawRect(
        markerRelX - 1,
        markerRelY + (bottomLabel ? 7 : -12),
        markerRelX + 1,
        markerRelY + (bottomLabel ? 12 : -7),
        "#ffffff"
      );

      // Draw label
      let text = marker.name.includes("%s")
        ? marker.name.replace("%s", marker.speed)
        : marker.name;
      this.drawCenteredText(
        markerRelX,
        markerRelY + (bottomLabel ? 26 : -18),
        text,
        enabled ? "#ffffff" : "#aaaaaa",
        12,
        true
      );
    ...
    let stateString =
      currentSpeed === 1 ? "ACTUAL SPEED" : currentSpeed + "x SPEED";
  ...
  drawTelemetry(satellite, x, y, width, height, date) {
    ...
    if (state.hasCrashed()) {
      this.drawText(
        ...
        y + height - 8,
        satellite.name +
          " burnt up in the atmosphere and is no longer in orbit",
        "#999999",
        14,
        false
      );
      ...
      this.drawSpeedometerBackgroundCurve(
        curveStartX,
        y,
        width - curveStartX,
        height,
      ...
      this.drawCenteredText(
        x + 100 + (280 - 100) / 2,
        y + height - 8,
        satellite.name + " TELEMETRY(Developed by ermi.dev )",
        "#999999",
        14,
        false
      );
  ...
  drawSpeedometer(x, y, value, maxValue, title, unit) {
    ...
    let timePassed =
      this.satelliteTracker.getBrowserTime() -
      this.satelliteTracker.initializeTime;
    let initializeProgress = sigmoid(
      Math.max(0, Math.min(1, timePassed / initializationDuration)) * 10
    );

    // Progress position
    let arcTotalRange = Math.abs(arcEnd) + Math.abs(arcStart);
    let arcRange = Math.min(
      arcTotalRange,
      ((arcTotalRange - arcRedRange) / maxValue) * value * initializeProgress
    );
    let arcWhiteProgress = Math.min(arcTotalRange - arcRedRange, arcRange);
    let arcRedProgress =
      Math.max(arcTotalRange - arcRedRange, arcRange) -
      (arcTotalRange - arcRedRange);
    ...
    this.drawArc(
      ...
      y,
      radius,
      arcEnd - arcRedRange + arcRedProgress,
      arcEnd,
      arcThickness,
      "#aa0000"
    );
  ...
  drawArc(x, y, radius, degreeStart, degreeEnd, thickness, color) {
    ...
    this.context.arc(
      ...
      y,
      radius,
      (Math.PI / 180) * (degreeStart + 180),
      (Math.PI / 180) * (degreeEnd + 180)
    );
  ...
  drawSpeedometerBackgroundCurve(x, y, width, height, color) {
    ...
    this.context.bezierCurveTo(x1 + ax1, y1 + ay1, x2 - ax1, y2 - ay2, x2, y2);
  ...
  drawHint(
    ...
    if (timePassed > visibleDuration + initializationDuration) {
      ...
      initializeProgress -= Math.min(
        1.0,
        (1.0 / initializationDuration) * (timePassed - visibleDuration)
      );
    }
  ...
  drawAlignmentText(x, y, string, color, size, alignment, bold) {
    this.context.font =
      (bold ? "bold" : "normal") + " " + size + "px FoundryGridnik";
    this.context.fillStyle = color;
    this.context.textAlign =
      alignment === 0 ? "center" : alignment < 0 ? "left" : "right";
