window.SatelliteTracker = class {
  constructor(canvasWrapperId) {
    ...
    this.textureRegistry = new TextureRegistry(this, function (
      state,
      progress
    ) {
  ...
  createSatellites(earthGroup, foreground) {
    ...
    this.registry.loadLocalTLE(ISS.ID, function (tle) {
      let satellite = ISS.createSpacecraft(
        scope,
        tle,
        function (loaded, progress) {
          scope.loadingProgress.updateProgress(
            "model",
            loaded ? 100 : Math.min(99, progress)
          );
  ...
  toggleEarthFocus() {
    ...
    if (this.focusedEarth) {
      this.renderer.camera.position.y = Math.max(
        4853718,
  ...
  getTime() {
    let passed =
      (this.getBrowserTime() - this.prevTime) *
      (this.timeSpeed > -1.0 && this.timeSpeed < 0 ? 0 : this.timeSpeed);
